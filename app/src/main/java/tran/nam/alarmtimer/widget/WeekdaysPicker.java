...
public class WeekdaysPicker extends LinearLayout {
    ...
    private float dayViewPadding = 20;
    ...
    private void initView(AttributeSet attrs) {
        ...
        try {
            ...
            mBackgroundColor = a.getColor(R.styleable.WeekdaysPicker_background_color, Color.WHITE);
...
//            addView(spinner);
    ...
    private void createDayView(int tag, boolean selected) {
        ...
        dayView.setTag(tag);
        int margin = getDpFromPx(dayViewPadding);
...
//        dayView.setPadding(padding, padding, padding, padding);
    ...
    public void setSelectedDays(List<Integer> list) {
        for (int day = SUNDAY; day <= SATURDAY; day++) {
            if (weekend) {
                setDaySelected( findViewWithTag(day), list.contains(day));
            } else {
